...
def stacked_area(dead, constant, exploded, path):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x= dead.x, y= dead.y, mode='lines', line_color= 'rgba(255, 16, 0, .8)', fill= 'tozeroy')) # fill down to xaxis
    fig.add_trace(go.Scatter(x= constant.x, y= constant.y, mode='lines', line_color='rgba(250, 220, 0, .8)', fill='tonexty')) # fill to dead.y
    fig.add_trace(go.Scatter(x= exploded.x, y= exploded.y, mode='lines', line_color='rgba(38, 250, 1, .8)', fill='tonexty')) # fill to constant.y

...
Looking at the `stacked graph` it seems that the criteria to distinguish `exploded-signal network`(i.d. when the ratio output-10s/output-1s > 2; see https://github.com/Igor10798/Internship/blob/e5150c4d5500694d60f8b86cc782bf5710dc2570/first_network/analysis.py#L60-L68) is to be revised since sometimes a network with strong output is labelled as `Constant signal` as it is shown in the figure below.  
![alt text](https://github.com/Igor10798/Internship/blob/master/first_network/offline_analysis/params_interaction/stacked.png)  

### 12/07/2021
###### Validation of the network
I continued the work started in the lab: validating the network output. Since the bar graph representing the output in function of time for every pair of weight and probability was not as expected, I tried to investigate the output of the network eliminating the connections in function of input, comparing to the output of a single cell.  
After finding some differences for network composed by more than 2 neurons, I compared the case of network with `n = 3` and the single cell.
Firstly, I found some differences (https://github.com/Igor10798/Internship/blob/master/first_network/validation/net.png VS https://github.com/Igor10798/Internship/blob/master/first_network/validation/single.png). It seems that the neuron with `GID` 2 fires despite I truncated all the connections and the input was too low for making the single cell firing. On the other hand we can see that neuron `3` doesn't receive any input. It seems that neuron `2` receives a double input, while the other gets none.  
I printed connections with `nest.GetConnections` in order to check whether there were other unexpected connections, but nothing was found, this support the previous hypothesis.  
This phoenomenon is explainable because I used the rule `fixed_total_number: N` to connect the input to the neuron, which makes `N` random connection between the source and the target populations. This is good in order to create some stochasticity, but have the side effect to create some random difference when checking 2 conditions that should be identical.  
In order to validate this hypothesis I used `all_to_all` rule, that ensures a single connection between the input and all the neurons of the network. I got the results in (https://github.com/Igor10798/Internship/blob/master/first_network/validation/Vms.png && https://github.com/Igor10798/Internship/blob/master/first_network/validation/spikes.png VS https://github.com/Igor10798/Internship/blob/master/first_network/validation/Vms.png_alltoall && https://github.com/Igor10798/Internship/blob/master/first_network/validation/spikes_alltoall.png), which support the hypothesys I was testing.  
To further prove that the differences are to address to the type of rule used, I repeated plotted the output in function of the input using `all_to_all` rule rather than `fixed_total_number: N`, and I could observe the same exact output from the network and from the single cell (https://github.com/Igor10798/Internship/blob/master/first_network/validation/net_alltoall.png, https://github.com/Igor10798/Internship/blob/master/first_network/validation/single_alltoall.png).  
From this analysis I can conclude that my network behaves as expected, since neurons responds as I previously investigated in https://github.com/Igor10798/Internship/tree/master/g_L_investigation. The differences must be addressed to network properties rather than single neurons behavior.